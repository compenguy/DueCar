...
uint32_t HIDGamepad::Init(uint32_t parent, uint32_t port, uint32_t lowspeed)
{
  ...
  TRACE_USBHOST(printf("HIDGamepad::Init : number of configuration is %lu\r\n", num_of_conf);)

  for (uint32_t i = 0; i < num_of_conf; ++i)
  {
    ConfigDescParser<
      USB_CLASS_HID,
      HID_NO_INTF_SUBCLASS,
      HID_PROTOCOL_NONE,
  ...
  /* TODO: should we be setting idle?
  if (BOOT_PROTOCOL == 1)
  {
  ...
  */
  ...
  TRACE_USBHOST(printf("HIDGamepad::Init : HID device configured successfully\r\n");)
...
FailSetIdle:
  TRACE_USBHOST(printf("HIDGamepad::Init SetIdle : ");)
  goto Fail;
 ...
 */
void HIDGamepad::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep)
{
  ...
  if ((pep->bmAttributes & 0x03) == 3 && (pep->bEndpointAddress & 0x80) == 0x80)
  {
    ...
    TRACE_USBHOST(printf("HIDGamepad::EndpointXtract : Found new endpoint\r\n");)
    TRACE_USBHOST(printf("HIDGamepad::EndpointXtract : deviceEpNum: %lu\r\n", epInfo[index].deviceEpNum);)
    TRACE_USBHOST(printf("HIDGamepad::EndpointXtract : maxPktSize: %lu\r\n", epInfo[index].maxPktSize);)
    TRACE_USBHOST(printf("HIDGamepad::EndpointXtract : index: %lu\r\n", index);)
    ...
    if (pipe == 0)
    {
      TRACE_USBHOST(printf("HIDGamepad::EndpointXtract : Pipe allocation failure\r\n");)
      // Enumeration failed
      return;
    }
 ...
 */
void GamepadReportParser::Parse(HID *hid, bool is_rpt_id, uint32_t len, uint8_t *buf)
{
