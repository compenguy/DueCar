...
class Modem {
    ...
    public:
      ...
      ModulePin::output_state_t getConnectedModulePinOutputState(); // AT+AFTC? -> OK+Get:[0-3]
      void setConnectedModulePinOutputState(ModulePin::output_state_t); // AT+AFTC[0-3] -> OK+Set:[0-3]

      ModulePin::output_state_t getInitialModulePinOutputState(); // AT+BEFC? -> OK+Get:[0-3]
      void setInitialModulePinOutputState(ModulePin::output_state_t); // AT+BEFC[0-3] -> OK+Set:[0-3]
      ...
      void setCharacteristic(uint16_t); // AT+CHAR'0x'[0-F]{4} -> OK+Set:'0x'[0-F]{4}

      uint8_t getMinimumLinkLayerConnectionInterval(); // AT+COMI? -> OK+Get:[0-9]
      void setMinimumLinkLayerConnectionInterval(uint8_t); // AT+COMI[0-9] -> OK+Set:[0-9]

      uint8_t getMaximumLinkLayerConnectionInterval(); // AT+COMA? -> OK+Get:[0-9]
      void setMaximumLinkLayerConnectionInterval(uint8_t); // AT+COMA[0-9] -> OK+Set:[0-9]

      uint8_t getLinkLayerConnectionSlaveLatency(); // AT+COLA? -> OK+Get:[0-4]
      void setLinkLayerConnectionSlaveLatency(uint8_t); // AT+COLA[0-4] -> OK+Set:[0-4]
      ...
      Connection::response_t connectAddress(uint8_t[6]); // AT+CO[N1][0-2][0-9]{6} -> OK+CO[N1][N1][AEF]
      ...
      String getBeaconUuid(); // AT+IBE0? -> OK+Get:[0-F]{4}, AT+IBE1, AT+IBE2, AT+IBE3
      void setBeaconUuid(String&); // AT+IBE0[0-F]{4} -> OK+Set:[0-F]{4}, AT+IBE1, AT+IBE2, AT+IBE3
      ...
      void setSystemKeySetting(ModulePin::system_key_t); // AT+SYSK[0-1] -> OK+Set:[0-1]
